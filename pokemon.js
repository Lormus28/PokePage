...
async function pokemonData(url) {
  ...
  try {
    ...
    let typerute = localStorage.getItem("selectType") === "all"
    ? url 
    ...
    let pokemonjson = localStorage.getItem("selectType") === "all" 
    ? json.results
    : json.pokemon;
    ...
    $nextLink = json.next ? `<a class="nextPrevButton" href="${json.next}">➡️</a>` : "";
    $prevLink = json.previous ? `<a class= "nextPrevButton" href="${json.previous}">⬅️</a>` : "";
